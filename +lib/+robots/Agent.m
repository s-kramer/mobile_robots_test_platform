...
classdef Agent < lib.robots.Robot 
    properties
    end
    
    methods
        %Creates an Agent robot object
        %arg1 - handle to World
        %arg2 - initial struct={x,y,fi}
        %arg3 - range of view (not implemented)
        %arg4 - forcelaw object
        function obj = Agent(parent,state,varargin)
            ...
            elseif(numel(varargin)==2)
                obj.viewRange = 25;
                obj.forceLaw = varargin{1};
                obj.groups{end+1} = strcat('agent',int2str(varargin{2}));
